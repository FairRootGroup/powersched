...
def build_episode_summary_line(
    ...
    return (
        ...
        f"PropPower={_fmt_optional(episode_data.get('agent_prop_power_mwh'), 1)}/"
        f"{_fmt_optional(episode_data.get('baseline_prop_power_mwh'), 1)}/"
        f"{_fmt_optional(episode_data.get('baseline_off_prop_power_mwh'), 1)} MWh "
        f"(agent/base/base_off), "
        f"PropCost=€{_fmt_optional(episode_data.get('agent_prop_cost'), 0)}/"
        f"€{_fmt_optional(episode_data.get('baseline_prop_cost'), 0)}/"
        f"€{_fmt_optional(episode_data.get('baseline_off_prop_cost'), 0)} "
        f"(agent/base/base_off), "
        f"PropSavings=€{_fmt_optional(episode_data.get('savings_prop_cost_vs_baseline'), 0)}/"
        f"€{_fmt_optional(episode_data.get('savings_prop_cost_vs_baseline_off'), 0)}"
    )

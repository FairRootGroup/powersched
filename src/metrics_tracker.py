...
class MetricsTracker:
    ...
    def reset_episode_metrics(self):
        ...
        # Agent job metrics (episode-level)
        ...
        # Baseline job metrics (episode-level)
    ...
    def record_episode_completion(self, current_episode):
        ...
        avg_wait_time = self.episode_total_job_wait_time / self.episode_jobs_completed if self.episode_jobs_completed > 0 else 0
        baseline_avg_wait_time = self.episode_baseline_total_job_wait_time / self.episode_baseline_jobs_completed if self.episode_baseline_jobs_completed > 0 else 0

        # Calculate completion rates
        completion_rate = (self.episode_jobs_completed / self.episode_jobs_submitted * 100) if self.episode_jobs_submitted > 0 else 0
        baseline_completion_rate = (self.episode_baseline_jobs_completed / self.episode_baseline_jobs_submitted * 100) if self.episode_baseline_jobs_submitted > 0 else 0

        drop_rate = (self.episode_jobs_dropped / self.episode_jobs_submitted * 100) if self.episode_jobs_submitted else 0.0
        baseline_drop_rate = (self.episode_baseline_jobs_dropped / self.episode_baseline_jobs_submitted * 100) if self.episode_baseline_jobs_submitted else 0.0
